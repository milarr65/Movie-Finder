...
app.get("/", async (req, res) => {
	try {
		...
		const content = await renderPartial("index", {
...
app.get("/search", async (req, res) => {
	...
	try {
		...
		if (result.data.results.length === 0) {
			const content = await renderPartial("error", {
				message:
					"Seems that we couldn't find a match for your query. Check for any typos you may have missed.",
			});
			res.render("layout", { body: content });
		}

		const content = await renderPartial("search", {
			data: result.data.results,
		});
...
app.get("/details/movie/:id", async (req, res) => {
	try {
		...
		const params = {
			append_to_response: "recommendations,keywords,credits,videos",
		};
		const { general_info, recs, directors, keywords, videos } = await searchDetailsAPI(
			path,
			params,
			"movie"
		);
		...
		const content = await renderPartial("details-movie", {
			info: general_info,
			recs,
			keywords,
			directors,
...
app.get("/details/tv/:id", async (req, res) => {
	try {
		...
		const params = {
			append_to_response: "recommendations,content_ratings,videos,aggregate_credits,keywords",
		};
		const { general_info, recs, keywords, videos } = await searchDetailsAPI(path, params, "tv");

		const images = await getImages(path);

		const content = await renderPartial(`details-tv`, {
			info: general_info,
			recs,
			keywords,
			videos,
	...
	try {
		...
		if (media_type === "tv") {
			const general_info = {
				...
				first_air_date: data.first_air_date ? data.first_air_date.split("-")[0] : "NA", // Year only
				last_air_date: data.last_air_date ? data.last_air_date.split("-")[0] : "NA", // Year only
			...
			const recs = data.recommendations.results.map((item) => ({
				...
				year: item.first_air_date ? item.first_air_date.split("-")[0] : "NA",
				name: item.name,
			}));

			const keywords = data.keywords.results.map((item) => item.name);
			const videos = data.videos.results.map((item) => ({
				site: item.site,
				key: item.key,
			}));
			...
			const recs = data.recommendations.results.map((item) => ({
				...
				id: item.id,
				vote_average: item.vote_average.toFixed(1),
				year: item.release_date ? item.release_date.split("-")[0] : "NA",
				title: item.title,
			}));
			...
			const backdropsEn =
				data["images?language=en"]?.backdrops?.map((item) => item.file_path) || [];
			const backdropsNull =
				data["images?include_image_language=null"]?.backdrops?.map((item) => item.file_path) || [];
			const backdrops = [...backdropsEn, ...backdropsNull];

			const postersEn = data["images?language=en"]?.posters?.map((item) => item.file_path) || [];
			const postersNull =
				data["images?include_image_language=null"]?.posters?.map((item) => item.file_path) || [];
...
async function getImages(path) {
	try {
		const api_result = await axios.get(path + "/images", {
			params: {
				include_image_language: "en-US,es,null",
			},
			headers: api_headers,
		});
		const data = api_result.data;
		if (!data || (!data.posters && !data.backdrops)) {
			// console.log("No images found for the requested path: ", path);
			return [];
		}

		const posters = data.posters?.map((p) => p.file_path) || [];
		const backdrops = data.backdrops?.map((b) => b.file_path) || [];

		// console.log(`Found ${posters.length + backdrops.length} images for the requested path: `, path);
...
// HELPER FUNCTION FOR EJS LAYOUT
async function renderPartial(view, data = {}) {
	return new Promise((resolve, reject) => {
		ejs.renderFile(path.join(process.cwd(), "views", `${view}.ejs`), data, (err, str) => {
			if (err) reject(err);
			else resolve(str);
		});
	});
