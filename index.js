...
app.get("/details/movie/:id", async (req, res) => {
  try {
    ...
    const params = {
      append_to_response:
        "recommendations,keywords,credits,videos,images?include_image_language=null,images?language=en",
    };
    const { general_info, recs, directors, keywords, videos, backdrops, posters } =
      await searchDetailsAPI(path, params, "movie");
...
app.get("/details/tv/:id", async (req, res) => {
  try {
    ...
    const params = {
      append_to_response:
        "recommendations,content_ratings,videos,aggregate_credits,keywords,images?include_image_language=null,images?language=en",
    };
    const { general_info, recs, keywords, videos, backdrops, posters } = await searchDetailsAPI(
      path,
      params,
      "tv"
    );
      ...
      const general_info = {
        ...
        cast:
          data.aggregate_credits?.cast
            ...
            .sort((a, b) => {
              a.total_episode_count - b.total_episode_count;
            })
